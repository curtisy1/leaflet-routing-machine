...
export default class EventHub<T extends EventMap> implements IEventHub<T> {
  ...
  on<K extends EventKey<T>>(eventName: K, fn: EventReceiver<T[K]>) {
    this.eventTarget.addEventListener(eventName, (e) =>
      this.customEventFunctionWrapper(fn, (e as CustomEvent<T[K]>).detail),
    );
  }

  off<K extends EventKey<T>>(eventName: K, fn: EventReceiver<T[K]>) {
    this.eventTarget.removeEventListener(eventName, (e) =>
      this.customEventFunctionWrapper(fn, (e as CustomEvent<T[K]>).detail),
    );
  }
  ...
  private customEventFunctionWrapper<K extends EventKey<T>>(
