...
export enum InstructionModifier {
  Straight = 'Straight',
  SlightRight = 'SlightRight',
  Right = 'Right',
  SharpRight = 'SharpRight',
 ...
 */
export type IInstruction = IInstructionBase &
  (ITextInstruction | IDirectionInstruction);
...
export interface IRoutingError {
  ...
  status: number;
  url?: string;
  // biome-ignore lint/suspicious/noExplicitAny: target is an error type, so any is valid here
  target?: any;
}
